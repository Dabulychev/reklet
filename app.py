...
def initialize_database():
    """
    Creates only additional structures required by the application.
    Existing business tables are not deleted.
    """
    ...
    statements = [
        ...
        # ----------------------------------------------------
        """
        CREATE TABLE IF NOT EXISTS reklet.finished_goods_transactions (
            id serial4 PRIMARY KEY,
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_finished_goods_object
        ON reklet.finished_goods(object_id)
        """,

        """
        CREATE INDEX IF NOT EXISTS idx_finished_goods_item
        ...
        """
        CREATE INDEX IF NOT EXISTS idx_object_items_object
        ON reklet.object_items(object_id)
        """,

        """
        CREATE INDEX IF NOT EXISTS idx_object_items_template
...
# ============================================================
    ...
    if sub == "Object List":
        ...
        if not df.empty:
            ...
            if st.button("Save Changes", key="save_objects"):
        ...
        with st.form("create_object"):
            ...
            submit = st.form_submit_button("Create Object")
    ...
    # --------------------------------------------------------
        ...
        if objects.empty:
            ...
            object_map = {
                f"{row['id']} — {row['object_name']} — {row['client_name']}":
                    int(row["id"])
            ...
            st.subheader(
                f"{object_row['object_name']} / {object_row['client_name']}"
            )
            ...
            if not items.empty:
                ...
                st.info("No products in this object.")
            ...
            <p>
                <b>Object:</b>
                {escape(str(object_row['object_name'] or ''))}
            </p>
        ...
        if objects.empty:
            ...
            if items.empty:
                ...
                if requirements.empty:
                    ...
                    total_materials["shortage"] = (
                        total_materials["required_quantity"]
                        -
                        total_materials["stock_quantity"]
                    )
                    ...
                    for i, row in total_materials.iterrows():
                        ...
                        <tr>
                            <td>{i + 1}</td>
                            <td>{escape(str(row['material_name']))}</td>
                            <td>{escape(str(row['unit_name'] or ''))}</td>
                            <td>{float(row['required_quantity']):.4f}</td>
                            <td>{float(row['stock_quantity']):.4f}</td>
                            <td>{float(row['shortage']):.4f}</td>
                            <td>{float(row['material_cost']):.2f}</td>
                        </tr>
                        """
                    ...
                    st.download_button(
                        ...
                        data=material_html,
                        file_name=f"material_requirements_{object_id}.html",
                        mime="text/html"
                    )
...
# ============================================================
    ...
    with st.expander("Create Product Template", expanded=False):
    ...
    st.subheader("Product Material Specification")
    ...
    if templates.empty:
        ...
        if not specification.empty:
            ...
            st.markdown("### Delete Specification Row")

            delete_map = {
                f"{row['id']} — {row['material_name']}":
                    int(row["id"])
...
# ============================================================
    ...
    st.markdown("---")
    st.subheader("Material Issue to Production")
...
# ============================================================
    ...
    if objects.empty:
        ...
        if df.empty:
            ...
            st.markdown("---")
            st.subheader("Production Action")
            ...
            max_qty = (
                safe_int(item_row["qty_new"])
                if action == "Start Production"
                else safe_int(item_row["qty_production"])
            )
